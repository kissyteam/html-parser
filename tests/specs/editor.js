...
describe('html parser for editor', function () {
    it('can filter elementNames', function () {
        ...
        expect(writer.getHtml()).to.equal("xy");
    });
    ...
    it("can filter imagedata in vml@ie", function () {
        ...
        getTextSync('/tests/specs/vml_img.html', function (d) {
            before = d;
        });
        ...
        expect(html.indexOf('<img src="xx.jpg" title="me" />') !== -1).to.equal(true);
    });
    ...
    it('can filter attribute', function () {
        ...
        expect(writer.getHtml()).to.equal("<div>1</div><" + "b>2</b><span>3</span>");
    });
    ...
    it("can modify html structure on fly", function () {
        ...
        n.writeHtml(writer, filter);
        if (isIE()) {
            expect(writer.getHtml()).to.equal("<p>\xa0</p><p>1</p>");
        } else {
            expect(writer.getHtml()).to.equal("<p>&nbsp;<br /></p><p>1</p>");
        }
    });
    ...
    it("filter children works while modify html", function () {
        ...
        n.writeHtml(writer, filter);
        expect(writer.getHtml()).to.equal("<p>&nbsp;</p>");
    ...
    it("filter will run only once", function () {
        ...
        n.writeHtml(writer, filter);
        expect(writer.getHtml()).to.equal("<p><span></span></p>");
    ...
    it("can filter attributeNames", function () {
        ...
        n.writeHtml(writer, filter);
        expect(writer.getHtml()).to.equal("<p src=\"y\">x</p>");
    });
